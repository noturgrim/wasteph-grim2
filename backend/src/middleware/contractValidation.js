...
export const validateRequestContract = (req, res, next) => {
  try {
    // Parse signatories if it's a string (from FormData)
    const body = { ...req.body };
    if (body.signatories && typeof body.signatories === "string") {
      try {
        body.signatories = JSON.parse(body.signatories);
      } catch (e) {
        return next(new AppError("Invalid signatories format", 400));
      }
    }

    requestContractSchema.parse(body);
